...
class UniFormer(nn.Module):
    ...
    def __init__(self, depth=[3, 4, 8, 3], img_size=224, in_chans=3, num_classes=1000, embed_dim=[64, 128, 320, 512],
        ...
        self.radio_proj = nn.Linear(587, 512)
        # self.fuse = nn.Concat(1)

        # Classifier head
        self.head_1 = nn.Linear(embed_dim[-1], num_classes) if num_classes > 0 else nn.Identity()
        self.head_2 = nn.Linear(2*embed_dim[-1], num_classes) if num_classes > 0 else nn.Identity()
    ...
    def forward_features(self, x):
        ...
        # print ('patch_embed1 shape: ', x.shape)
        x = self.pos_drop(x)
        for blk in self.blocks1:
            x = blk(x)
        # print ('uniformer blk1 shape: ', x.shape)
        
        x = self.patch_embed2(x)
        # print ('patch_embed2 shape: ', x.shape)
        for blk in self.blocks2:
            x = blk(x)
        # print ('uniformer blk2 shape: ', x.shape)
        
        x = self.patch_embed3(x)
        # print ('patch_embed3 shape: ', x.shape)
        for blk in self.blocks3:
            x = blk(x)
        # print ('uniformer blk3 shape: ', x.shape)
        
        x = self.patch_embed4(x)
        # print ('patch_embed4 shape: ', x.shape)
        for blk in self.blocks4:
            x = blk(x)
        # print ('uniformer blk4 shape: ', x.shape)
    ...
    def forward(self, x, r, mode):
        ...
        if mode == 0:
            ...
            # print ('mode 1')
            r = r.squeeze()
            r = self.radio_proj(r.to(torch.float))
            if r.dim() == 1:
                r = r.unsqueeze(0)
