...
parser.add_argument('--radiomics_fea_file', default='', type=str)       ###add 20240709
parser.add_argument('--use_radio', type=int, default=0)
...
def train_one_epoch(
    ...
    for batch_idx, (input, target, radio_fea) in enumerate(loader):
        last_batch = batch_idx == last_idx
        data_time_m.update(time.time() - end)
        # print (input.shape, radio_fea.shape)
        input, target, radio_fea = input.cuda(), target.cuda(), radio_fea.cuda()
        with amp_autocast():
            output = model(input, radio_fea, args.use_radio)
