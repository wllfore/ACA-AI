...
class UniFormer(nn.Module):
    ...
    def __init__(self, depth=[3, 4, 8, 3], img_size=224, in_chans=3, num_classes=1000, embed_dim=[64, 128, 320, 512],
        ...
        self.proj1 = PatchEmbed(
                img_size=img_size, patch_size=3, in_chans=in_chans, embed_dim=embed_dim[0], stride=1, padding=1)

        
        self.out_1 = UnetOutBlock(spatial_dims=3, in_channels=64, out_channels=num_classes)
    ...
    def forward_features(self, x):
        ...
        # print ('output x: ', x4.shape)

        dec5 = self.decoder5(x4.permute(0,1,3,4,2), x3.permute(0,1,3,4,2))
        # print ('dec5: ', dec5.shape)

        dec4 = self.decoder4(dec5, x2.permute(0,1,3,4,2))
        # print ('dec4: ', dec4.shape)

        dec3 = self.decoder3(dec4, x1.permute(0,1,3,4,2))
...
def uniformer_base_IL(num_classes=2, 
    ...
    Concat multi-phase images with image-level
    '''
    print ('num_classes = ', num_classes, ', num_phase = ', num_phase)
